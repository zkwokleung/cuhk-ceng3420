...
int label_to_num(int line_no, char *label, int width, struct_label_table *label_table, int number_of_labels)
{
...
void la_to_binary(FILE *output_file, int line_no, char *opcode, char *arg1, char *arg2, char *arg3,
                  struct_label_table *label_table, int number_of_labels, int addr)
{
    /*
    ...
    char temp[MAX_LINE_LENGTH + 1];
    fprintf(output_file, "0x%08x\n",
            inst_to_binary(line_no, "lui", arg1, arg2, arg3, label_table, number_of_labels, addr));
    sprintf(temp, "%d", (handle_label_or_imm(line_no, arg2, label_table, number_of_labels)) & 0xFFF);
    fprintf(output_file, "0x%08x\n",
            inst_to_binary(line_no, "addi", arg1, arg1, temp, label_table, number_of_labels, addr));
}

int inst_to_binary(int line_no, char *opcode, char *arg1, char *arg2, char *arg3, struct_label_table *label_table,
                   int number_of_labels, int addr)
{
    ...
    else if (is_opcode(opcode) == LUI)
    {
        binary = (0x0D << 2) + 0x03;
        binary += (reg_to_num(arg1, line_no) << 7);
        binary += handle_label_or_imm(line_no, arg2, label_table, number_of_labels) & 0xFFFFF000;
    }
...
int is_label(char *ptr)
{
    ...
    char *label = ptr;
    if (is_opcode(label) == MISMATCH && is_reg(ptr) == MISMATCH && is_imm(ptr) == MISMATCH &&
...
int parse_inst(FILE *input_file, char **label, char **opcode, char **arg1, char **arg2, char **arg3)
{
    char *ptr;
    int i;
    ...
    *label = *opcode = *arg1 = *arg2 = *arg3 = line + strlen(line);
    ...
    case 1:
         ...
         */
        ptr = strtok(line, "\t\n ,");
        assert(cmp_str(opcode_table[30], ptr), "halt is expected in the input asm source code.\n");
        *opcode = ptr;
        return TYPE1;
    case 2:
        /*
        ...
        assert(is_label(ptr) == VALID_LABEL, "a valid label is expected in the input asm source code.\n");
        *label = ptr;
    ...
    case 3:
        ...
        if (is_label(ptr) == VALID_LABEL)
        {
            *label = ptr;
            ptr = strtok(NULL, delimiters);
            assert(is_opcode(ptr) != MISMATCH, "a valid label is expected in the input asm source code.\n");
            *opcode = ptr;
            ptr = strtok(NULL, delimiters);
            assert(is_arg(ptr) != MISMATCH, "opcode is expected in the input asm source code.\n");
            *arg1 = ptr;
            return TYPE3;
        }
        else
        {
            assert(is_opcode(ptr) != MISMATCH, "opcode is expected in the input asm source code.\n");
            *opcode = ptr;
            ptr = strtok(NULL, delimiters);
            assert(is_arg(ptr) != MISMATCH, "argument is expected in the input asm source code.\n");
            *arg1 = ptr;
            ptr = strtok(NULL, delimiters);
            assert(is_arg(ptr) != MISMATCH, "argument is expected in the input asm source code.\n");
            *arg2 = ptr;
...
bool if_no_duplicated_label(struct_label_table *label_table, char *label, int line_no)
{
    for (int i = 0; i < MAX_LABELS; i++)
    {
        if (cmp_str(label_table[i].label, label))
        {
            ...
            error("line: %d, label '%s' has a conflict with other labels in line %d.\n", line_no, label,
...
void insert_a_label(struct_label_table *label_table, struct_label_table **head_of_label_table, char *label, int line_no,
                    int addr)
{
...
void assemble(char *input_file_name, char *output_file_name)
{
    ...
    // handle labels
    while ((ret = parse_inst(input_file, &label, &opcode, &arg1, &arg2, &arg3)) != END_OF_FILE)
    {
        line_no++;
    ...
    for (int i = 0; i < number_of_labels; i++)
    {
        test("label: %s, address: %d, line number: %d\n", label_table[i].label, label_table[i].address,
             label_table[i].line_no);
    }
#endif
    ...
    while ((ret = parse_inst(input_file, &label, &opcode, &arg1, &arg2, &arg3)) != END_OF_FILE)
    {
        ...
        else if (ret == TYPE4 || ret == TYPE5)
        {
            ...
            {
                la_to_binary(output_file, line_no, opcode, arg1, arg2, arg3, label_table, number_of_labels, addr);
            }
            addr += 0x4;
        }
