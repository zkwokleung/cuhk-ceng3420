...
void eval_bus_drivers()
{
    ...
    int value_of_MARMUX = 0, value_of_alu, value_of_shift_function_unit = 0;

    value_of_MARMUX =
        addr2_mux(
            get_ADDR2MUX(CURRENT_LATCHES.MICROINSTRUCTION), 0, sext_unit(mask_val(CURRENT_LATCHES.IR, 31, 20), 12),
            sext_unit(s_format_imm_gen_unit(mask_val(CURRENT_LATCHES.IR, 11, 7), mask_val(CURRENT_LATCHES.IR, 31, 25)),
                      12),
            sext_unit(j_format_imm_gen_unit(mask_val(CURRENT_LATCHES.IR, 31, 31), mask_val(CURRENT_LATCHES.IR, 30, 21),
                                            mask_val(CURRENT_LATCHES.IR, 20, 20), mask_val(CURRENT_LATCHES.IR, 19, 12)),
                      20)) +
        addr1_mux(
            get_ADDR1MUX(CURRENT_LATCHES.MICROINSTRUCTION), 0, CURRENT_LATCHES.PC,
            rs1_en(get_RS1En(CURRENT_LATCHES.MICROINSTRUCTION),
                   CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 19, 15)]),
            sext_unit(b_format_imm_gen_unit(mask_val(CURRENT_LATCHES.IR, 7, 7), mask_val(CURRENT_LATCHES.IR, 11, 8),
                                            mask_val(CURRENT_LATCHES.IR, 30, 25), mask_val(CURRENT_LATCHES.IR, 31, 31)),
                      12));
    ...
    /* input of GateALUSHF */
    value_of_GateALUSHF =
        alu_shift_mux(mask_val(CURRENT_LATCHES.IR, 14, 12), value_of_alu, value_of_shift_function_unit);
    ...
    /* input of GateRS2 */
    value_of_GateRS2 =
        rs2_en(get_RS2En(CURRENT_LATCHES.MICROINSTRUCTION), CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 24, 20)]);
